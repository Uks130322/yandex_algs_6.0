...
def find_max_branch_outside_diameter(tree, diameter_path):
    ...
    def dfs(node, parent):
        nonlocal max_outside_branch, max_outside_node
        max_branch = 0
        ...
        for child in tree[node]:
            ...
            child_branch = dfs(child, node) + 1
            max_branch = max(max_branch, child_branch)

        max_branches[node] = max_branch

        if max_branch > max_outside_branch:
            max_outside_branch = max_branch
            max_outside_node = node

        return max_branch

    for node in range(len(tree)):
        if node not in diameter_set:
            dfs(node, -1)
